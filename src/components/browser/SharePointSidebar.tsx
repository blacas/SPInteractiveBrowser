...
import { X, ChevronRight, ChevronDown, Home, Search, MoreVertical, Download, FolderOpen, File, Loader2, RefreshCw, ArrowLeft } from 'lucide-react';
...
import { sharepointService, SharePointFile, SharePointSite, SharePointDrive, BreadcrumbItem } from '../../services/sharepointService';
...
export const SharePointSidebar: React.FC<SharePointSidebarProps> = ({
  ...
  const [dragPreviews, setDragPreviews] = useState<Map<string, DragPreviewData>>(new Map());
  ...
  useEffect(() => {
    const initializeService = async () => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : 'Failed to initialize SharePoint service');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const loadFiles = async (siteId: string, driveId: string, folderId?: string) => {
    setIsLoading(true);
    setError(null);
  ...
  const handleRefresh = async () => {
    ...
    await loadFiles(selectedSite.id, selectedDrive.id, currentFolderId || undefined);
    setIsRefreshing(false);
  };

  // Pre-load file for drag operations
  const preloadFileForDrag = useCallback(async (file: SharePointFile) => {
    ...
    try {
      ...
      console.log(`✅ Pre-loaded ${file.name} (${sharepointService.formatFileSize(blob.size)})`);
    } catch (error) {
  ...
  const handleDragStart = useCallback((e: React.DragEvent, file: SharePointFile) => {
    if (file.isFolder) {
      e.preventDefault();
      return;
    }
    ...
    if (cachedData?.isReady && cachedData.blob) {
      try {
        ...
        const fileObject = new File([cachedData.blob], file.name, { 
          type: cachedData.blob.type || 'application/octet-stream' 
        });
  ...
  const handleDragEnd = useCallback((e: React.DragEvent, file: SharePointFile) => {
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="text-xs text-gray-500 dark:text-gray-400">
              {file.isFolder ? 'Folder' : sharepointService.formatFileSize(file.size)}
            </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold">
                {showDiagnostics ? 'SharePoint Diagnostics' : 'SharePoint Files'}
              </h2>
                ...
                <React.Fragment key={item.id || index}>
                  ...
                  <span
                    className={cn(
                      ...
                      index === breadcrumb.length - 2 ? "text-blue-600 dark:text-blue-400 font-medium" : "text-gray-600 dark:text-gray-400"
                    )}
          ...
          {showDiagnostics ? (
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <p className="text-gray-600 dark:text-gray-400">Loading SharePoint files...</p>
              </div>
            </div>
          ) : error ? (
            <div className="p-4">
